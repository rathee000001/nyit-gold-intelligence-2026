...
const STARTER_SUGGESTIONS = [
  ...
  "Why is high_yield excluded?",
  "Explain the final forecast in professor-safe wording.",
];

function modeLabel(mode?: string) {
  if (!mode) return "AI";
  if (mode === "project_artifact_ai") return "Artifact AI";
  if (mode === "general_ai") return "General AI";
  if (mode === "artifact_fallback") return "Artifact Fallback";
  if (mode === "needs_api_key") return "Needs API Key";
  if (mode === "api_error") return "API Error";
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      role: "assistant",
      content:
        "Hi — I am the Gold Nexus Alpha interpreter. Ask me about the project artifacts, models, final forecast, cutoff, factors, or general forecasting concepts.",
      mode: "project_artifact_ai",
      sources: [],
    },
  ]);
    ...
    try {
      ...
      setMessages((current) => [
        ...current,
        {
          ...
          content:
            data.answer ||
            "I could not generate an answer from the available artifacts.",
          mode: data.mode,
        ...
        ...current,
        {
          ...
          content:
            error instanceof Error
              ? `Interpreter connection error: ${error.message}`
              : "Interpreter connection error.",
          mode: "error",
          sources: [],
        },
      ]);
  ...
  return (
    <>
      <button
        ...
        aria-label="Open Gold Nexus Alpha interpreter"
      >
        <RobotIcon />
        <div className="hidden text-left md:block">
          ...
          <p className="text-sm font-black">Ask the project</p>
        </div>
      </button>

      {open ? (
        <div className="fixed bottom-5 right-5 z-[9999] flex h-[min(760px,calc(100vh-40px))] w-[min(460px,calc(100vw-28px))] flex-col overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-[0_30px_120px_rgba(15,23,42,0.35)]">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs leading-5 text-slate-300">
                    Artifact-grounded project answers + general AI guidance.
                  </p>
                </div>
              </div>
          ...
          <div className="flex-1 space-y-4 overflow-y-auto bg-slate-50 p-4">
              ...
              <div
                ...
                className={
                  message.role === "user"
                    ...
                    : "mr-auto max-w-[92%] rounded-[1.3rem] border border-slate-200 bg-white p-4 text-slate-900 shadow-sm"
                }
              >
            ...
            {busy ? (
              <div className="mr-auto max-w-[92%] rounded-[1.3rem] border border-slate-200 bg-white p-4 text-slate-700 shadow-sm">
          ...
          <div className="border-t border-slate-200 bg-white p-4">
            ...
            <div className="flex items-end gap-2 rounded-3xl border border-slate-200 bg-slate-50 p-2">
              <textarea
                ...
                placeholder="Ask about the forecast, models, factors, or a general concept..."
                rows={2}
            ...
            <p className="mt-2 text-[11px] leading-5 text-slate-500">
              Project claims are grounded in approved JSON artifacts. General
              answers are labeled separately.
            </p>
          </div>
        </div>
      ) : null}
